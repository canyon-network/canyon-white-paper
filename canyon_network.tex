...
Canyon is a permanent storage network built on Substrate, which records the hashes of files on-chain and stores the actual files off-chain. By blending PoS and a probabilistic proof-of-storage scheme inspired by Arweave, Canyon is able to pledge a pretty high data durability in theory and greatly reduces the barriers to entry for storage miners who are incentivized to store as much data as possible for winning more rewards, rendering Canyon Network a highly sustainable decentralized storage platform .
